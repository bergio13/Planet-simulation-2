...
class Object:              
    def __init__(self, name, rad, color, radius, velocity):
        self.name = name
        self.radius = np.array(radius, dtype=float)
        self.velocity = np.array(velocity, dtype=float)
        self.xs = []
        self.ys = []
        self.plot = ax.scatter(radius[0], radius[1], color=color, s=rad**2, edgecolors=None, zorder=10)
...
class SolarSystem:
    ...
    def evolve(self):           
        ...
        for planet in self.planets:
            planet.radius += planet.velocity * dt
            acc = -2.959e-4 * planet.radius / np.sum(planet.radius**2)**(3./2)  # in units of AU/day^2
            planet.velocity += acc * dt
            planet.xs.append(planet.radius[0])
            planet.ys.append(planet.radius[1])
            planet.plot.set_offsets(planet.radius[:2])
